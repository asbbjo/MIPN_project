...
public class BrainRRT : MonoBehaviour
{
    ...
    public GameObject endObject; // End object assigned in the Inspector
    public GameObject boundaryObstacle; // Boundary object assigned in the Inspector
    public List<GameObject> obstacles; // List of all obstacle objects assigned in the Inspector
    public int numberOfPoints; // Maximal number of points in the RRT
    public int numberOfBranches; // Number of branches in each RRT branch
    public float branchDistance; // Maximal distance to search for points
    public int thresholdFactor; // Factor of branchDistance to avoid obstacles
    private List<GameObject> newPoints = new List<GameObject>();
    private List<GameObject> prunedPoints = new List<GameObject>();
    private List<GameObject> smoothObjects = new List<GameObject>();
    private List<Vector3> NewPointsSmoothed = new List<Vector3>();
    private List<Vector3> NewPointsSmoothedFinal = new List<Vector3>();
    ...
    {   
        ...
        // Add mesh colliders to all objects
    ...
    void GenerateRRTPoints(GameObject start, GameObject target, int numberOfPoints, float branchDistance, int numberOfBranches){
    GameObject currentObject = start;
    for (int i = 0; i < numberOfPoints; i++){
        Vector3 newPoint = new Vector3(
        UnityEngine.Random.Range(currentObject.transform.position.x - branchDistance, currentObject.transform.position.x + branchDistance),
        UnityEngine.Random.Range(currentObject.transform.position.y - branchDistance, currentObject.transform.position.y + branchDistance),
        UnityEngine.Random.Range(currentObject.transform.position.z - branchDistance, currentObject.transform.position.z + branchDistance));
        ...
        if (Vector3.Distance(currentObject.transform.position, target.transform.position) < branchDistance/2){
    ...
    float threshold_obstacles = distance * thresholdFactor;
                ...
                for(int j = 0; j < smoothedPath.Count-1; j++){
                    ...
                    if((Vector3.Distance(smoothedPath[j],smoothedPath[j+1]))>10){
                        if(IsObstacleBetween(ObjectA, ObjectB)){
                            UnityEngine.Debug.Log("Hits an obstacle");
                            D32 = D32*0.5f; 
                            D02 = D02*0.5f; 
                            HitsObstacle=true;
                            count++;
